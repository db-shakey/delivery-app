_app.directive("outputField", function(MetadataFactory, $filter, $timeout, JSUtils){
	return {
		restrict : 'E',
		scope : {
			record : "="
		...
		link : function($scope, element, attributes){
			...
			if(fieldName.indexOf('__r') != -1){
				objectName = fieldName.substring(0, fieldName.indexOf('__r') + 2) + 'c';
				fieldName = fieldName.substring(fieldName.indexOf('__r') + 4);
			}

			$scope.$watch("record", function(newValue){
				$scope.value = JSUtils.getDescendantProp($scope.record, attributes.fieldName, attributes.currency);
			});

			$scope.field = attributes.fieldLabel;
			//$scope.value = JSUtils.getDescendantProp($scope.record, attributes.fieldName, attributes.currency);
			$scope.layout = attributes.layout;

			/*
			MetadataFactory.describe(objectName, function(metaFields){
				$timeout(function(){
					$scope.field = $filter('filter')(metaFields, {name : fieldName})[0];
					$scope.value = JSUtils.getDescendantProp($scope.record, attributes.fieldName, attributes.currency);
					$scope.layout = attributes.layout;
				});
			});
...
_app.directive("haversine", function($filter){
	return {
		restrict : 'A',
		link : function($scope, element, attributes){
			...
			var lat2 = Number(attributes.latTo);
			var lon2 = Number(attributes.lonTo);
			var lat1 = Number(attributes.latFrom);
			var lon1 = Number(attributes.lonFrom);
			...
			var d = R * c;
			d /= 1.60934;;
			$(element).html('<i class="icon ion-ios-navigate-outline"></i>&nbsp;' + $filter('number')(d, 1) + " mi");
		}
	}
})


_app.directive('autoFocus', function($timeout) {
    return {
        restrict: 'AC',
...
_app.directive('fallbackSrc', function () {
